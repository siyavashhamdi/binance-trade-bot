...
export class TradeInfoReverse {
    ...
    public listenOpenOrderChanges(pollingBySec = 10000) {
        setInterval(async () => {
            const openOrdersCount: number = (await this.getOpenOrders('BUY')).length;
            ...
            if (openOrdersCount < this.currentOpenOrdersCount) {
                ...
                this.telegram?.sendBroadcastMessage(`Reverse: Open order count changed:\n${ msgBalances }`);
            }
        }, pollingBySec);
    }
...
`;
    ...
    public async orderPlanA(objInput: any): Promise<void> {
        ...
        if (resBuy?.status === 'FILLED') {
            ...
            utils.consoleLog(`Reverse: Buy order created on price ${ priceToBuy.toFixed(8) }${ this.cryptoPair.dst } with amount ${ investAmountByDst }${ this.cryptoPair.src }`);
...
Buy order created on price ${ priceToSell.toFixed(8) } ${ this.cryptoPair.dst } with amount ${ investAmountByDst } ${ this.cryptoPair.dst } `;
